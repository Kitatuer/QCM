...
function renderAdminDashboard() {
  ...
  elements.participantResults.replaceChildren(...displayedResults
    .slice()
    .sort((a, b) => b.percent - a.percent || compareElapsedTimes(a.elapsedMs, b.elapsedMs) || new Date(b.date) - new Date(a.date))
    .map((result, index) => renderParticipantResult(result, index)));
}
...
function renderParticipantResult(result, rank = 0) {
  const article = document.createElement("article");
  ...
  article.innerHTML = `
    <header>
      <h3><span class="rank-number">#${rank + 1}</span> ${escapeHtml(result.participantName)}</h3>
