...
const TIMER_START_KEY = "qcm-studio-timer-start";
const PARTICIPANT_ROSTER_KEY = "qcm-studio-participant-roster";
const PARTICIPANT_ROSTER_DOC = "_participant_roster";
...
let startedAt = isParticipantMode ? loadTimerStart() : null;
let participantStarted = !isParticipantMode || Boolean(participantName && startedAt);
let lastResult = null;
let adminResults = loadAdminResults();
let participantRoster = loadParticipantRoster();
...
let adminResultsMode = "recent";
let onlineSaveTimer = null;
let hasAutoRestoredOnlineQuiz = false;
let participantRosterOnlineAvailable = false;
let onlineStore = {
  enabled: false,
  ready: false,
  error: "",
...
const elements = {
  ...
  views: {
    ...
    participants: document.querySelector("#participantsView")
  },
  ...
  participantIdentityMode: document.querySelector("#participantIdentityMode"),
  ...
  participantNameLabel: document.querySelector("#participantNameLabel"),
  participantSelect: document.querySelector("#participantSelect"),
  participantSelectLabel: document.querySelector("#participantSelectLabel"),
  ...
  resultsModeTitle: document.querySelector("#resultsModeTitle"),
  showRecentResults: document.querySelector("#showRecentResults"),
  showRanking: document.querySelector("#showRanking"),
  participantRosterForm: document.querySelector("#participantRosterForm"),
  newParticipantName: document.querySelector("#newParticipantName"),
  participantRosterSummary: document.querySelector("#participantRosterSummary"),
  participantRosterList: document.querySelector("#participantRosterList"),
...
document.querySelector("#sessionFilter").addEventListener("change", (event) => {
  selectedSessionId = event.target.value;
  renderAdminDashboard();
});
document.querySelector("#showRecentResults").addEventListener("click", () => {
  adminResultsMode = "recent";
  renderAdminDashboard();
});
document.querySelector("#showRanking").addEventListener("click", () => {
  adminResultsMode = "ranking";
  renderAdminDashboard();
});
document.querySelector("#participantRosterForm").addEventListener("submit", addParticipantToRoster);
document.querySelector("#participantName").addEventListener("input", (event) => {
  if (event.target.value.trim()) elements.participantSelect.value = "";
});
document.querySelector("#participantSelect").addEventListener("change", (event) => {
  if (event.target.value) elements.participantName.value = "";
});
...
function loadQuiz(sharedQuiz) {
  ...
  try {
    ...
    return normalizeQuiz(structuredClone(sampleQuiz));
  }
}
...
function loadTimerStart() {
  try {
    const saved = Number(sessionStorage.getItem(getTimerStartKey()));
...
function loadParticipantRoster() {
  try {
    const saved = JSON.parse(localStorage.getItem(PARTICIPANT_ROSTER_KEY));
    return normalizeParticipantRoster(saved);
  } catch {
    return [];
  }
}

function normalizeParticipantRoster(value) {
...
function normalizeQuiz(value) {
  if (!value || typeof value !== "object") return normalizeQuiz(structuredClone(sampleQuiz));

  const normalized = {
    id: String(value.id || crypto.randomUUID()),
    sessionId: String(value.sessionId || ""),
    sessionName: String(value.sessionName || "").slice(0, 120),
    ...
    participantIdentityMode: ["free", "list", "both"].includes(value.participantIdentityMode) ? value.participantIdentityMode : "free",
    participantNames: Array.isArray(value.participantNames)
      ? Array.from(new Set(value.participantNames.map((name) => String(name).trim()).filter(Boolean))).slice(0, 200)
      : [],
...
function expandSharedQuiz(data) {
  ...
  return {
    ...
    description: data.d,
    passScore: data.p,
    participantIdentityMode: data.im,
    participantNames: data.pn,
    questions: Array.isArray(data.q) ? data.q.map((question) => ({
      ...
      choices: Array.isArray(question.c) ? question.c.map((choice) => ({
        text: Array.isArray(choice) ? choice[0] : choice.text,
        correct: Boolean(Array.isArray(choice) ? choice[1] : choice.correct)
      })) : []
    })) : []
  };
}
...
function normalizeResult(result) {
  ...
  return {
    ...
    sessionId: String(result.sessionId || `legacy-${result.quizId || "unknown"}`),
    sessionName: String(result.sessionName || result.quizTitle || "Session historique").slice(0, 120),
    ...
    elapsedMs: result.elapsedMs == null ? null : Math.max(0, Number(result.elapsedMs)),
...
function renderBuilder() {
  ...
  elements.participantIdentityMode.value = quiz.participantIdentityMode;
...
function renderPlayer() {
  ...
  const timerLine = isParticipantMode && participantStarted ? `<br>Temps : <strong id="quizTimer">${formatDuration(getElapsedMs())}</strong>` : "";
  elements.playerIntro.innerHTML = `<strong>${escapeHtml(quiz.title || "QCM sans titre")}</strong><br>${escapeHtml(intro)}${participantLine}${timerLine}<br><span id="answerProgress">${answered}/${quiz.questions.length} question(s) répondues.</span>`;
  startTimerDisplay();
...
function renderResults() {
  ...
  elements.scoreBoard.innerHTML = `
    ...
    <span>Temps total : <b>${formatDuration(lastResult.elapsedMs)}</b></span>
  `;
  renderResultActions();
...
function syncQuizMeta() {
  ...
  quiz.participantIdentityMode = ["free", "list", "both"].includes(elements.participantIdentityMode.value)
    ? elements.participantIdentityMode.value
    : "free";
  syncQuizParticipantRoster();
  renderParticipantIdentityGate();
  saveAndRefreshLight();
}
...
async function copyShareLink() {
  syncQuizParticipantRoster();
  if (quiz.participantIdentityMode === "list" && !quiz.participantNames.length) {
    setStatus("Ajoute au moins un participant avant de copier le lien.");
    return;
  }
...
function createQuizSharePayload() {
  ...
  const payload = {
    ...
    d: quiz.description,
    p: quiz.passScore,
    im: quiz.participantIdentityMode,
    pn: quiz.participantNames,
...
function renderAdminDashboard() {
  ...
  const displayedResults = selectedSessionId === "all"
    ? adminResults
    : adminResults.filter((result) => result.sessionId === selectedSessionId);
  const total = displayedResults.length;
  const passed = displayedResults.filter((result) => result.passed).length;
  const average = total ? Math.round(displayedResults.reduce((sum, result) => sum + result.percent, 0) / total) : 0;
  const mistakes = displayedResults.reduce((sum, result) => sum + result.details.filter((detail) => !detail.isCorrect).length, 0);
  ...
  if (!displayedResults.length) {
    elements.participantResults.innerHTML = `<div class="empty-state">Aucun résultat pour cette session.</div>`;
    return;
  }

  const sortedResults = displayedResults.slice().sort(adminResultsMode === "ranking"
    ? (a, b) => b.percent - a.percent || compareElapsedTimes(a.elapsedMs, b.elapsedMs) || new Date(b.date) - new Date(a.date)
    : (a, b) => new Date(b.date) - new Date(a.date));
  elements.participantResults.replaceChildren(...sortedResults
    .map((result, index) => renderParticipantResult(result, index, adminResultsMode === "ranking")));
}

function renderResultsMode() {
  const isRanking = adminResultsMode === "ranking";
  elements.resultsModeTitle.textContent = isRanking ? "Classement de la session" : "Dernières participations";
  elements.showRecentResults.classList.toggle("active", !isRanking);
  elements.showRanking.classList.toggle("active", isRanking);
}

function renderSessionFilter() {
  if (!elements.sessionFilter) return;

  const sessions = new Map();
  adminOnlineQuizzes.forEach((item) => {
    if (item.sessionId) sessions.set(item.sessionId, item.sessionName || item.title);
  });
  adminResults.forEach((result) => {
    sessions.set(result.sessionId, result.sessionName || result.quizTitle);
  });

  if (selectedSessionId !== "all" && !sessions.has(selectedSessionId)) {
    selectedSessionId = "all";
  }

  const options = [`<option value="all">Toutes les sessions</option>`];
  Array.from(sessions.entries())
    .sort((a, b) => a[1].localeCompare(b[1], "fr"))
    .forEach(([sessionId, sessionName]) => {
      options.push(`<option value="${escapeHtml(sessionId)}">${escapeHtml(sessionName)}</option>`);
    });
  elements.sessionFilter.innerHTML = options.join("");
  elements.sessionFilter.value = selectedSessionId;
}

function renderParticipantRoster() {
  if (!elements.participantRosterList) return;

  const enabledCount = participantRoster.filter((item) => item.enabled).length;
  elements.participantRosterSummary.innerHTML = `<strong>${enabledCount}</strong> participant(s) sélectionné(s) sur ${participantRoster.length}.`;

  if (!participantRoster.length) {
    elements.participantRosterList.innerHTML = `<div class="empty-state">Ajoute des noms pour utiliser les modes Liste imposée ou Liste + nom libre.</div>`;
    return;
  }

  elements.participantRosterList.replaceChildren(...participantRoster
    .slice()
    .sort((a, b) => a.name.localeCompare(b.name, "fr"))
    .map(renderParticipantRosterItem));
}

function renderParticipantRosterItem(item) {
  const row = document.createElement("div");
  row.className = "participant-roster-item";
  const checkboxId = `roster-${item.id}`;
  row.innerHTML = `
    <input id="${checkboxId}" type="checkbox" ${item.enabled ? "checked" : ""} aria-label="Activer ${escapeHtml(item.name)}">
    <label for="${checkboxId}">${escapeHtml(item.name)}</label>
    <button class="icon-button" type="button" aria-label="Supprimer ${escapeHtml(item.name)}" title="Supprimer">
      <svg viewBox="0 0 24 24"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/></svg>
    </button>
  `;
  row.querySelector("input").addEventListener("change", (event) => {
    item.enabled = event.target.checked;
    saveParticipantRoster();
  });
  row.querySelector("button").addEventListener("click", () => {
    participantRoster = participantRoster.filter((participant) => participant.id !== item.id);
    saveParticipantRoster();
  });
  return row;
}

function addParticipantToRoster(event) {
  event.preventDefault();
  const name = elements.newParticipantName.value.trim();
  if (!name) {
    elements.newParticipantName.focus();
    return;
  }
  const existing = participantRoster.find((item) => item.name.localeCompare(name, "fr", { sensitivity: "accent" }) === 0);
  if (existing) {
    existing.enabled = true;
    setStatus("Participant déjà présent et réactivé.");
  } else {
    participantRoster.push({ id: crypto.randomUUID(), name: name.slice(0, 80), enabled: true });
    setStatus("Participant ajouté.");
  }
  elements.newParticipantName.value = "";
  saveParticipantRoster();
  elements.newParticipantName.focus();
}

function saveParticipantRoster() {
  participantRoster = normalizeParticipantRoster(participantRoster);
  localStorage.setItem(PARTICIPANT_ROSTER_KEY, JSON.stringify(participantRoster));
  syncQuizParticipantRoster();
  renderParticipantRoster();
  renderParticipantIdentityGate();
  if (onlineStore.ready) saveParticipantRosterOnline();
}

function getEnabledParticipantNames() {
...
function getAvailableParticipantNames() {
  if (onlineQuizIdFromUrl && participantRosterOnlineAvailable) return getEnabledParticipantNames();
  if (!isParticipantMode) return getEnabledParticipantNames();
  return (Array.isArray(quiz.participantNames) ? quiz.participantNames : [])
    .slice()
    .sort((a, b) => a.localeCompare(b, "fr", { sensitivity: "base" }));
}

function syncQuizParticipantRoster() {
  if (!isParticipantMode) quiz.participantNames = getEnabledParticipantNames();
}
...
function renderOnlineQuizCard(quizItem) {
  ...
  article.innerHTML = `
    <div>
      ...
      <p>Session : ${escapeHtml(quizItem.sessionName || "Aucune session")}</p>
    </div>
    <div class="online-quiz-actions">
      ...
      <button class="quiet-action new-session" type="button">Nouvelle session</button>
  ...
  article.querySelector(".new-session").addEventListener("click", () => createNewQuizSession(quizItem));
  article.querySelector(".toggle-online-quiz").addEventListener("click", () => toggleOnlineQuiz(quizItem.id, !quizItem.active));
  return article;
}

function renderParticipantResult(result, rank = 0, showRank = false) {
  const article = document.createElement("article");
  ...
  article.innerHTML = `
    <header>
      <h3>${showRank ? `<span class="rank-number">#${rank + 1}</span> ` : ""}${escapeHtml(result.participantName)}</h3>
      <span class="pill ${result.passed ? "ok" : "bad"}">${result.percent}%</span>
    </header>
    <div class="participant-meta">
      <span><b>QCM :</b> ${escapeHtml(result.quizTitle)}</span>
      <span><b>Session :</b> ${escapeHtml(result.sessionName)}</span>
      ...
      <span><b>Temps :</b> ${formatDuration(result.elapsedMs)}</span>
    </div>
...
async function loadParticipantRosterOnline() {
  if (!onlineStore.ready) return;
  try {
    const { doc, getDoc } = onlineStore.api;
    const snapshot = await getDoc(doc(onlineStore.db, "qcmQuizzes", PARTICIPANT_ROSTER_DOC));
    if (!snapshot.exists()) return;
    participantRoster = normalizeParticipantRoster(snapshot.data().participants);
    participantRosterOnlineAvailable = true;
    localStorage.setItem(PARTICIPANT_ROSTER_KEY, JSON.stringify(participantRoster));
    renderParticipantRoster();
  } catch (error) {
    console.warn(error);
    setStatus("Lecture de la liste des participants impossible.");
  }
}

async function saveParticipantRosterOnline() {
  if (!onlineStore.ready) return;
  try {
    const { doc, serverTimestamp, setDoc } = onlineStore.api;
    await setDoc(doc(onlineStore.db, "qcmQuizzes", PARTICIPANT_ROSTER_DOC), {
      kind: "participantRoster",
      participants: participantRoster,
      updatedAt: serverTimestamp()
    });
    participantRosterOnlineAvailable = true;
    setStatus("Liste des participants enregistrée en ligne.");
  } catch (error) {
    console.warn(error);
    setStatus("Enregistrement de la liste impossible.");
  }
}
...
async function publishCurrentQuiz(active = true, options = {}) {
  ...
  syncQuizParticipantRoster();
  if (quiz.participantIdentityMode === "list" && !quiz.participantNames.length) {
    if (!options.silent) setStatus("Ajoute au moins un participant avant de publier en mode liste.");
    return false;
  }

  try {
    const { doc, serverTimestamp, setDoc } = onlineStore.api;
    const existing = adminOnlineQuizzes.find((item) => item.id === quiz.id);
    quiz.sessionId = quiz.sessionId || existing?.sessionId || crypto.randomUUID();
    quiz.sessionName = quiz.sessionName || existing?.sessionName || createDefaultSessionName(quiz.title);
    ...
    if (!options.skipRefresh) await refreshOnlineQuizzes();
    return true;
  } catch (error) {
    console.warn(error);
...
function schedulePublishedQuizSave() {
  if (!onlineStore.ready || isParticipantMode) return;
  const published = adminOnlineQuizzes.find((item) => item.id === quiz.id);
  if (!published) return;

  window.clearTimeout(onlineSaveTimer);
  onlineSaveTimer = window.setTimeout(() => {
    publishCurrentQuiz(published.active, { silent: true, skipRefresh: true });
  }, 1200);
}
...
async function refreshOnlineQuizzes() {
  ...
  await maybeRestorePublishedQuiz();
  renderAdminDashboard();
...
async function loadOnlineQuizLibrary() {
  ...
  try {
    ...
    snapshot.forEach((docSnapshot) => {
      ...
      if (data.kind === "participantRoster" || docSnapshot.id === PARTICIPANT_ROSTER_DOC) return;
      const item = {
        id: docSnapshot.id,
        title: data.title || "QCM sans titre",
        description: data.description || "",
        active: Boolean(data.active),
        sessionId: String(data.sessionId || `legacy-${docSnapshot.id}`),
        sessionName: String(data.sessionName || data.title || "Session historique"),
...
async function maybeRestorePublishedQuiz() {
  if (hasAutoRestoredOnlineQuiz || isParticipantMode || !isDefaultSampleQuiz(quiz) || !adminOnlineQuizzes.length) return;

  const latest = adminOnlineQuizzes
    .slice()
    .sort((a, b) => Number(b.active) - Number(a.active) || b.updatedAt - a.updatedAt)[0];
  if (!latest) return;

  hasAutoRestoredOnlineQuiz = true;
  await loadOnlineQuizIntoAdmin(latest.id, { silent: true });
  setStatus(`QCM publié restauré : ${latest.title}`);
}

function isDefaultSampleQuiz(value) {
  return value?.title === sampleQuiz.title
    && value?.questions?.length === sampleQuiz.questions.length
    && value.questions[0]?.text === sampleQuiz.questions[0]?.text;
}
...
async function loadOnlineQuizById(quizId) {
  ...
  try {
    ...
    quiz.id = snapshot.id;
    quiz.sessionId = String(data.sessionId || `legacy-${snapshot.id}`);
    quiz.sessionName = String(data.sessionName || data.title || "Session historique");
    if (participantRosterOnlineAvailable) quiz.participantNames = getEnabledParticipantNames();
    answers = {};
    lastResult = null;
...
async function createNewQuizSession(quizItem) {
  if (!onlineStore.ready) return;

  const defaultName = createDefaultSessionName(quizItem.title);
  const sessionName = prompt("Nom de la nouvelle session :", defaultName)?.trim();
  if (!sessionName) return;

  try {
    const { doc, serverTimestamp, updateDoc } = onlineStore.api;
    const sessionId = crypto.randomUUID();
    await updateDoc(doc(onlineStore.db, "qcmQuizzes", quizItem.id), {
      active: true,
      sessionId,
      sessionName: sessionName.slice(0, 120),
      updatedAt: serverTimestamp()
    });
    if (quiz.id === quizItem.id) {
      quiz.sessionId = sessionId;
      quiz.sessionName = sessionName.slice(0, 120);
      saveState();
    }
    selectedSessionId = sessionId;
    setStatus("Nouvelle session créée et activée.");
    await refreshOnlineQuizzes();
  } catch (error) {
    console.warn(error);
    setStatus("Création de la session impossible.");
  }
}

function createDefaultSessionName(quizTitle) {
  return `${quizTitle || "QCM"} - ${new Date().toLocaleString("fr-FR")}`;
}

async function loadOnlineQuizIntoAdmin(quizId, options = {}) {
  const item = adminOnlineQuizzes.find((quizItem) => quizItem.id === quizId);
  if (!item?.payload) return;

  quiz = normalizeQuiz(expandSharedQuiz(item.payload));
  quiz.id = item.id;
  quiz.sessionId = item.sessionId;
  quiz.sessionName = item.sessionName;
  answers = {};
  lastResult = null;
  currentQuestionIndex = 0;
  render();
  switchView("builder");
  if (!options.silent) setStatus("QCM chargé dans l'éditeur.");
}
...
async function initOnlineStorage() {
  ...
  try {
    ...
    onlineStore.error = "";
    await loadParticipantRosterOnline();
...
function renderParticipantIdentityGate() {
  const mode = quiz.participantIdentityMode || "free";
  const allowFree = mode === "free" || mode === "both";
  const allowList = mode === "list" || mode === "both";
  const availableNames = getAvailableParticipantNames();

  elements.participantNameLabel.classList.toggle("is-hidden", !allowFree);
  elements.participantSelectLabel.classList.toggle("is-hidden", !allowList);

  const options = [`<option value="">Choisir un nom</option>`];
  availableNames.forEach((name) => {
    options.push(`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`);
  });
  elements.participantSelect.innerHTML = options.join("");

  if (allowList && availableNames.includes(participantName)) {
    elements.participantSelect.value = participantName;
    if (mode === "list") elements.participantName.value = "";
  } else if (allowFree) {
    elements.participantName.value = participantName;
  }
}
...
function startQuiz() {
  const mode = quiz.participantIdentityMode || "free";
  const typedName = elements.participantName.value.trim();
  const selectedName = elements.participantSelect.value.trim();
  participantName = mode === "list" ? selectedName : (mode === "both" ? typedName || selectedName : typedName);
  if (!participantName) {
    if (mode === "list") {
      elements.participantSelect.focus();
      setStatus("Choisis ton nom dans la liste.");
    } else {
      elements.participantName.focus();
      setStatus("Entre ou sélectionne un nom.");
    }
    return;
  }
...
function submitAnswers() {
  ...
  lastResult = {
    id: crypto.randomUUID(),
    quizId: getQuizId(),
    sessionId: quiz.sessionId || `legacy-${getQuizId()}`,
    sessionName: quiz.sessionName || quiz.title || "Session historique",
...
function retryQuiz() {
  ...
  lastResult = null;
  currentQuestionIndex = 0;
  startedAt = isParticipantMode ? Date.now() : null;
  if (startedAt) saveTimerStart();
  saveState();
  render();
  switchView("player");
}
...
function switchView(name) {
  ...
  if (name === "participants") renderParticipantRoster();
...
function getTimerStartKey() {
  return `${TIMER_START_KEY}-${hashText(location.hash)}`;
}
...
function saveTimerStart() {
  if (!isParticipantMode || !startedAt) return;
  try {
    sessionStorage.setItem(getTimerStartKey(), String(startedAt));
  } catch {
    // Le chronomètre continue en mémoire si le stockage de session est bloqué.
  }
}

function clearTimerStart() {
  try {
    sessionStorage.removeItem(getTimerStartKey());
  } catch {
...
function startTimerDisplay() {
  stopTimerDisplay();
  if (!isParticipantMode || !participantStarted || !startedAt) return;
...
function formatDuration(milliseconds) {
  if (milliseconds == null || !Number.isFinite(Number(milliseconds))) return "Non mesuré";
...
function compareElapsedTimes(first, second) {
