...
function loadQuiz() {
  const sharedQuiz = readQuizFromUrl();
  if (sharedQuiz) {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(sharedQuiz));
    return sharedQuiz;
  }
...
function renderPlayer() {
  ...
  elements.playerIntro.innerHTML = `<strong>${quiz.title || "QCM sans titre"}</strong><br>${escapeHtml(quiz.description || "Sélectionne les réponses, puis lance la correction.")}<br>${answered}/${quiz.questions.length} question(s) répondues.`;
...
function renderResults() {
  ...
  elements.scoreBoard.innerHTML = `
    ...
    <span>${lastResult.correct}/${lastResult.total} bonne(s) réponse(s). Score attendu: ${quiz.passScore}%.</span>
  `;
...
function createShareLink() {
  ...
  const base64 = btoa(unescape(encodeURIComponent(json))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
  return `${location.origin}${location.pathname}${location.search}#qcm=${base64}`;
}
