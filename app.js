...
async function loadOnlineResults() {
  ...
  try {
    const { collection, getDocs, query, where } = onlineStore.api;
    const snapshot = await getDocs(query(collection(onlineStore.db, "qcmResults"), where("quizId", "==", getQuizId())));
...
function getQuizId() {
  return hashText(JSON.stringify(createQuizSharePayload()));
}
