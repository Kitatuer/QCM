...
const elements = {
  ...
  participantNamesSetting: document.querySelector("#participantNamesSetting"),
...
function renderBuilder() {
  ...
  elements.participantNames.value = quiz.participantNames.join("\n");
  renderParticipantNamesSetting();
...
function syncQuizMeta() {
  ...
  quiz.participantIdentityMode = ["free", "list", "both"].includes(elements.participantIdentityMode.value)
    ...
    : "free";
  quiz.participantNames = parseParticipantNames(elements.participantNames.value);
  renderParticipantNamesSetting();
  renderParticipantIdentityGate();
  saveAndRefreshLight();
}

function renderParticipantNamesSetting() {
  elements.participantNamesSetting.classList.toggle("is-hidden", quiz.participantIdentityMode === "free");
}
...
function renderParticipantIdentityGate() {
  ...
  quiz.participantNames.forEach((name) => {
    options.push(`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`);
  });
  elements.participantSelect.innerHTML = options.join("");

  if (allowList && quiz.participantNames.includes(participantName)) {
